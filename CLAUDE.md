...
```
JWT_SECRET=<min 32 char secret for JWT signing>   # the only variable with no default. There is deliberately no fallback: a key committed to the repo would be identical in every clone. Unset or under 32 chars exits 1 with an actionable message rather than a panic.
...
PORTS=80,8082                         # comma-separated HTTP listen ports. Every port is bound up front; a port that fails to bind (typically :80 as a non-root user) logs a warning and is reported via CaptureException, and startup only fails if NO port binds. Order is not significant.
